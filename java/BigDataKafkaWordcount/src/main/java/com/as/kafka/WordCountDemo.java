...
public final class WordCountDemo {
    ...
    //o preparamos una función más especializada por ejemplo la func splitter
    //import re
    //def splitter(line):
    //   line = re.sub(r'^\W+|\W+$', '', line)
    //   return map(str.lower, re.split(r'\W+', line))
    //words= linesRDD.flatMap(splitter) #Tokenización
    //words.collect()
    //pairkey_1= words.map(lambda x: (x,1))
    //counts = pairkey_1.reduceByKey( lambda x,y: x+y)
    ...
    public static void main(final String[] args) {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread("streams-wordcount-shutdown-xx") {
            @Override
            public void run() {
                streams.close();
                latch.countDown();
